...
String htmlManagementHeader() {
  return String(F("<!DOCTYPE html><html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><title>DIY BMS Management Console</title><script type=\"text/javascript\" src=\"https://a-malik31.github.io/diyBMS_newUI/loader.js\"></script></head><body></body></html>\r\n\r\n"));
}
...
void handleSetEmonCMS() {
  ...
  myConfig.temp_setpoint = server.arg("temp_setpoint").toFloat();
  myConfig.temp_hysteresis = server.arg("temp_hysteresis").toFloat();
...
void handleSettingsJSON() {
  String json1 =   "{\"emoncms_enabled\":" + (myConfig.emoncms_enabled ? String("true") : String("false"))
                   ...
                   +",\"temp_setpoint\":\"" + String(myConfig.temp_setpoint) + "\""
                   +",\"temp_hysteresis\":\"" + String(myConfig.temp_hysteresis) + "\""
                   + "}\r\n\r\n";
  server.send(200, "application/json", json1 );
}
...
void handleCellJSONData() {
  if (simulationMode) {
    ...
    float current = 0;
    float temp = 25;
    bool cutoff_status = false;
    bool tempcontrol = false;

    String json6 = String(temp);
    String json7 = String(current);
    String json8 = String(cutoff_status);
    String json9 = String(tempcontrol);

    server.send(200,"application/json",
    "["+json1+","+json2+","+json3+","+json4+","+json5+","+json6+","+json7+","+json8+","+json9+"]");
  ...
  String json6 = "[" + String(temp_nano) + "]";
  String json7 = "[" + String(AmpsRMS) + "]";

  server.send(200, "application/json","[" + json1 + "," + json2 + "," + json3 + "," + json4 + "," + json5 + "," + json6 + "," + json7 + "]\r\n\r\n");
}
