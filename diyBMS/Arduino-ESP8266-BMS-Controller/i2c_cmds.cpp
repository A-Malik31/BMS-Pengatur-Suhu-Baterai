...
float read_temperature_from_nano()
{
  float value=0.0;

  Wire.requestFrom((uint8_t)NANO_ADDR, (uint8_t)4);
...
void send_temp_correction(float corr)
{
  byte *data = (byte*)&corr;

  Wire.beginTransmission(NANO_ADDR);

  Wire.write(0x02);   // command set correction
